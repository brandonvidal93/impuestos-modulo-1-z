...
class ItemInteractiveAudio extends Component {
  ...
  makeItem = () => {
    const ITEM = this.props.dataItem[this.state.pageItem].map( (item, i) => {
      return(
        <div id = { item.time } className = { 'itemInfo animated fadeIn dNone ' + (item.labelSide === 'left' ? 'dF-Rr-cc' : 'dF-C-cc') + (item.last === true ? ' lastItem' : ' item') } key = { i } style = { { 'top': item.position.yPosition, 'left': item.position.xPosition, 'width': item.widthBox } }>
          <img
            alt = 'Imagen Item'
            className = 'imageItem'
            style = { { 'width': item.size } }
            src = { item.icon }/>
          <h5 className = 'fw-3 mB-05' dangerouslySetInnerHTML = { { __html: item.label } } />
        </div>
      );
    } );
    this.showItem(ITEM);
