    ...
    if (actualIndex !== 0) {
      return(
        <div className = 'buttonPannel'>
          ...
          <button
            className = { 'buttonNav ' + (actualIndex === 2 || actualIndex === limitNavigation || Object.values(data)[actualIndex].endCourse === true ? 'disabled ': '') + (actualIndex === 5 || actualIndex === 10 || actualIndex === 15 || actualIndex === 19 ? 'op-4' : '') }
            id = 'btnNavRight'
            onClick = { this.navigationCourse }>
            <FontAwesomeIcon
              className = 'iconButton'
